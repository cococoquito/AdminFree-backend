...
public class SQLSeguridad {
	...
	public static final String INICIAR_SESION_USER = "SELECT US.ID_USUARIO AS ID_USUARIO, US.NOMBRE AS NOMBRE_USUARIO, CL.ID_CLIENTE AS ID_CLIENTE, CL.NOMBRE AS NOMBRE_CLIENTE, MODULO.ID_MODULO AS ID_MODULO, MODULO.NOMBRE AS NOMBRE_MODULO FROM USUARIOS US JOIN CLIENTES CL ON(CL.ID_CLIENTE = US.CLIENTE) JOIN USUARIOS_MODULOS USER_MODULO ON(USER_MODULO.ID_USUARIO = US.ID_USUARIO) JOIN MODULOS MODULO ON(MODULO.ID_MODULO = USER_MODULO.ID_MODULO) WHERE US.USUARIO_INGRESO = ? AND US.CLAVE_INGRESO = ? AND US.ESTADO = ? AND CL.ESTADO = ?";
