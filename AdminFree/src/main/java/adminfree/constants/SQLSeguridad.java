...
public class SQLSeguridad {
	...
	public static final String INICIAR_SESION_USER = "SELECT US.ID_USUARIO,US.NOMBRE,CL.ID_CLIENTE,CL.NOMBRE,USER_MODULO.TOKEN_MODULO FROM USUARIOS US JOIN CLIENTES CL ON(CL.ID_CLIENTE = US.CLIENTE) JOIN USUARIOS_MODULOS USER_MODULO ON(USER_MODULO.ID_USUARIO = US.ID_USUARIO) JOIN MODULOS M ON(M.TOKEN_MODULO = USER_MODULO.TOKEN_MODULO) WHERE US.USUARIO_INGRESO=? AND US.CLAVE_INGRESO=? AND US.ESTADO=? AND CL.ESTADO=? ORDER BY M.ORDEN";
