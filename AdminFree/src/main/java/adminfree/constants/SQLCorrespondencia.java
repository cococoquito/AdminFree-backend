...
public class SQLCorrespondencia {
	...
	public static StringBuilder getSQLSelectConsecutivosAnioActual(StringBuilder from) {
		StringBuilder sql = new StringBuilder("SELECT CON.ID_CONSECUTIVO,CON.CONSECUTIVO,NOM.NOMENCLATURA,COALESCE(US.NOMBRE, 'Administrador') AS USUARIO,DATE_FORMAT(CON.FECHA_SOLICITUD,'%d - %b - %Y') AS FECHA_SOLI, CON.ESTADO");
		sql.append(from);
		return sql;
	}
	...
	public static String getSQLConsecutivo(String idCliente, String idConsecutivo) {
		StringBuilder sql = new StringBuilder("SELECT CON.ID_CONSECUTIVO,CON.CONSECUTIVO,NOM.NOMENCLATURA,COALESCE(US.NOMBRE, 'Administrador')AS USUARIO,DATE_FORMAT(CON.FECHA_SOLICITUD,'%d - %b - %Y')AS FECHA_SOLI,CON.ESTADO,NOM.DESCRIPCION FROM CONSECUTIVOS_");
		sql.append(idCliente);
