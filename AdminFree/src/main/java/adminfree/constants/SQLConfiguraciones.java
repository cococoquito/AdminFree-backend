...
public class SQLConfiguraciones {
	...
	public static final String GET_DETALLE_CAMPO_ENTRADA = "SELECT CE.ID_CAMPO AS ID_CAMPO, CE.CLIENTE AS ID_CLIENTE, CE.NOMBRE AS NOMBRE_CAMPO, CE.DESCRIPCION AS DESCRIPCION_CAMPO, CE.TIPO_CAMPO AS TIPO_CAMPO, R.ID_RESTRICCION AS ID_RES, R.NOMBRE AS NOMBRE_RES, R.DESCRIPCION AS DESCRIPCION_RES, I.ID_ITEM AS ID_ITEM, I.VALOR AS ITEM_VALOR FROM CAMPOS_ENTRADA CE LEFT JOIN CAMPOS_ENTRADA_RESTRICCIONES RES ON (RES.CAMPO = CE.ID_CAMPO) LEFT JOIN RESTRICCIONES R ON (R.ID_RESTRICCION = RES.RESTRICCION) LEFT JOIN SELECT_ITEMS I ON(I.CAMPO = CE.ID_CAMPO) WHERE CE.ID_CAMPO =?";
}
